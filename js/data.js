...
class DataManager {
    ...
    filter(tableName, filterField, filterValue) {
        ...
        return items.filter(item => 
            item[filterField] && item[filterField].toString().toLowerCase() === filterValue.toLowerCase()
        );
    }
    ...
    getTable(tableType) {
        ...
        if (data.length === 0) {
            html += '<div class="text-center p-3"><p>No data available. Click "Add New" to get started.</p></div>';
        }
