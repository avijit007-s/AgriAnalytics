    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'create':
                ...
                $stmt->execute([$name, $type, $variety, $sowing_time, $transplanting_time, $harvest_time, $seed_per_acre]);
            ...
            case 'update':
                ...
                $stmt = $conn->prepare($sql);
                $stmt->execute([$name, $type, $variety, $sowing_time, $transplanting_time, $harvest_time, $seed_per_acre, $product_id]);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $type_labels = [];
    $type_data = [];
    if ($type_result && $type_result->rowCount() > 0) {
    ...
    $seed_labels = [];
    $seed_data = [];
    if ($seed_result && $seed_result->rowCount() > 0) {
